...
owoify(Source, Level) ->
    ...
    Words = lists:map(fun(W) ->
        Match = lists:nth(1, W),
        #word{inner_word = Match, replaced_words = sets:new()}
    end, WordMatches),
    SpaceMatches = case re:run(Source, space_regex(), [global, {capture, all, binary}]) of
        {match, Matches} -> Matches;
        _ -> []
    end,
    Spaces = lists:map(fun(S) ->
        Match = lists:nth(1, S),
        #word{inner_word = Match, replaced_words = sets:new()}
    end, SpaceMatches),
    Presets = owoify_presets:specific_word_mapping_list() ++ (case Level of
        owo -> owoify_presets:owo_mapping_list();
        uwu -> owoify_presets:uwu_mapping_list() ++ owoify_presets:owo_mapping_list();
        uvu -> owoify_presets:uvu_mapping_list() ++ owoify_presets:uwu_mapping_list() ++ owoify_presets:owo_mapping_list();
        _ -> error(incorrect_owoness)
    end),
    Applied = lists:map(fun(W) ->
...
%% @private
-spec word_regex() -> {'re_pattern', _, _, _, _}.
word_regex() ->
    {ok, Mp} = re:compile("[^\s]+", [unicode]),
    Mp.

%% @private
-spec space_regex() -> {'re_pattern', _, _, _, _}.
space_regex() ->
    {ok, Mp} = re:compile("\s+", [unicode]),
    Mp.